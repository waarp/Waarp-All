...
public class TYPE extends AbstractCommand {
  ...
  public void exec() throws Reply501Exception, Reply504Exception {
    ...
    if (transferType == FtpArgumentCode.TransferType.ASCII) {
      ...
      getSession().getDataConn().setType(FtpArgumentCode.TransferType.IMAGE);
    } else {
      throw new Reply504Exception(
          "Type not implemented: " + transferType.name());
    }
    ...
    if (types.length > 2) {
      ...
      for (int i = 1; i < types.length; i++) {
        if (types[i].length() != 0) {
          try {
            transferSubType =
                FtpArgumentCode.getTransferSubType(types[i].charAt(0));
          } catch (final InvalidArgumentException e) {
          ...
          if (transferSubType != TransferSubType.NONPRINT) {
            throw new Reply504Exception(
                "Format Control not implemented: " + transferSubType.name());
          }
        }
      }
    }
