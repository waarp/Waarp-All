  ...
  public void exec() throws Reply501Exception, Reply504Exception {
    ...
    if (!hasArg()) {
      getSession().getDataConn()
                  .setStructure(FtpArgumentCode.TransferStructure.FILE);
    ...
    if (transferStructure == FtpArgumentCode.TransferStructure.FILE) {
      ...
      getSession().getDataConn()
                  .setStructure(FtpArgumentCode.TransferStructure.RECORD);
    } else {
      throw new Reply504Exception(
          "Structure not implemented: " + transferStructure.name());
    }
    getSession().setReplyCode(ReplyCode.REPLY_200_COMMAND_OKAY,
