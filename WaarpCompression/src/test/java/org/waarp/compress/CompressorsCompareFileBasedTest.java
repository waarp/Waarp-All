...
public class CompressorsCompareFileBasedTest {
  ...
  private static byte[] readFileToBytes(final File file) throws IOException {
    ...
    try {
      ...
      while (len < bytes.length &&
             (read = fis.read(bytes, len, bytes.length - len)) > 0) {
        len += read;
      }
    } finally {
      if (fis != null) {
  ...
  public void test(final CompressorCodec compressor,
      ...
      throws IOException {
    byte[] bytes = readFileToBytes(files.get(0));
    byte[] compressed;
    byte[] decompressed;
