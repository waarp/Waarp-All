...
import static org.waarp.compress.zstdsafe.UnsafeUtil.getLong;
...
final class XxHash64 {
  ...
  public static long hash(final long seed, final byte[] base, final int address,
                          final int length) {
    long hash;
    if (length >= 32) {
  ...
  private static long updateTail(long hash, final byte[] base,
                                 final int address, int index,
                                 final int length) {
    while (index <= length - 8) {
      hash = updateTail(hash, getLong(base, address + index));
      index += 8;
    }

    if (index <= length - 4) {
      hash = updateTail(hash, getInt(base, address + index));
      index += 4;
    }
  ...
  private static long updateBody(final long seed, final byte[] base,
                                 int address, final int length) {
    ...
    while (remaining >= 32) {
      v1 = mix(v1, getLong(base, address));
      v2 = mix(v2, getLong(base, address + 8));
      v3 = mix(v3, getLong(base, address + 16));
      v4 = mix(v4, getLong(base, address + 24));
