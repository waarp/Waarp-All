...
import static org.waarp.compress.zstdunsafe.FiniteStateEntropy.*;
...
class SequenceEncoder {
  ...
  public static int compressSequences(final Object outputBase,
    ...
    if (sequenceCount < 0x7F) {
      ...
      output++;
      UNSAFE.putShort(outputBase, output,
    ...
    final long headerAddress = output++;
    ...
    // flags
    UNSAFE.putByte(outputBase, headerAddress,
                   (byte) ((literalsLengthEncodingType << 6) |
  ...
  private static int selectEncodingType(final int largestCount,
    ...
    if (strategy.ordinal() < CompressionParameters.Strategy.LAZY
      ...
      if (isDefaultTableAllowed) {
        ...
        final int baseLog = 3;
        final long minNumberOfSequences =
            ((1L << defaultNormalizedCountsLog) * factor) >>
