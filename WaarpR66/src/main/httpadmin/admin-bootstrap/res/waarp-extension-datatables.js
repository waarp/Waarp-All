...
var _saveAs = (function(view) {
	...
	var doc = view.document
		...
		, revoke = function(file) {
			...
			if (view.chrome) {revoker();
			} else {setTimeout(revoker, arbitrary_revoke_timeout);}
		}
			...
			req_fs(view.TEMPORARY, fs_min_size, abortable(function(fs) {
				fs.root.getDirectory("saved", create_if_not_found, abortable(function(dir) {
					...
					dir.getFile(name, {create: false}, abortable(function(file) {
						...
						if (ex.code === ex.NOT_FOUND_ERR) {save();} else {fs_error();}
					}));
				}), fs_error);
			}), fs_error);
		}
		, FS_proto = FileSaver.prototype
		, saveAs = function(blob, name) {return new FileSaver(blob, name);}
	;
	// IE 10+ (native saveAs)
	if (typeof navigator !== "undefined" && navigator.msSaveOrOpenBlob) {
		return function(blob, name) {return navigator.msSaveOrOpenBlob(auto_bom(blob), name);};
	}
...
var _title = function ( config ){
	...
	return title.indexOf( '*' ) !== -1 ? title.replace( '*', $('title').text() ) : title;
};
/**
...
var _exportData = function ( dt, config ){
	...
	var body = [];
	for ( var i=0, ien=data.body.length ; i<ien ; i++ ) { body.push( join( data.body[i] ) ); }
	return { str: header + body.join( newLine ) + footer, rows: body.length };
};
/**
...
DataTable.ext.buttons.copyHtml5 = {
	...
	text: function ( dt ) {return dt.i18n( 'buttons.copy', 'Copy' );},
	action: function ( e, dt, button, config ) {
		...
		$(document)
			...
			.on( 'copy.buttons-copy cut.buttons-copy', function () {close();} );
	},
	exportOptions: {},
	fieldSeparator: '\t',
	fieldBoundary: '',
...
DataTable.ext.buttons.csvHtml5 = {
	...
	available: function () {return window.FileReader !== undefined && window.Blob;},
	text: function ( dt ) {return dt.i18n( 'buttons.csv', 'CSV' );},
	action: function ( e, dt, button, config ) {
		// Set the text
		var newLine = _newLine( config );
...
DataTable.ext.buttons.excelHtml5 = {
	...
	text: function ( dt ) {return dt.i18n( 'buttons.excel', 'Excel' );},
	action: function ( e, dt, button, config ) {
		// Set the text
		var xml = '';
		var data = dt.buttons.exportData( config.exportOptions );
		...
		var addCell = function ( cells, data ) {
		...
		var addRowBody = function ( row ) {
			var cells = [];
			var newlines = [];
			for ( var i=0, ien=row.length ; i<ien ; i++ ) {
				if ( row[i] === null || row[i] === undefined ) {row[i] = '';}
				if (Object.prototype.toString.call( row[i] ) === '[object Array]') {
					var len = row[i].length;
					// Put header
					var headd = row[i][0];
					var fieldlen = headd.length;
					var newhead = [];
					for (var j = 0; j < fieldlen ; j++) {
						addCell(newhead, headd[j]+'');
					}
					newlines.push(newhead);
					for (var k = 1; k < len ; k++) {
						var newline = [];
						var rowd = row[i][k];
						for (var j = 0; j < fieldlen ; j++) {
							if (headd[j].search("TIME") == -1) { addCell(newline, rowd[headd[j]]+''); }
							else { addCell(newline, renderTimestamp(rowd[headd[j]])); }
						}
						newlines.push(newline);
					}
					cells.push('<c t="n"><v>'+(len-1)+'</v></c>');
				} else {
					// Don't match numbers with leading zeros or a negative anywhere
					// but the start
					cells.push( typeof row[i] === 'number' || (row[i].match && row[i].match(/^-?[0-9\.]+$/) && row[i].charAt(0) !== '0') ?
			...
			if (newlines.length > 0) {
				...
				for (var k = 0; k < newlines.length ; k++) {
					nestedtable += '<row><c t="inlineStr"><is><t> </t></is></c>'+newlines[k].join('')+'</row>';
				}
				return '<row>'+cells.join('')+'</row>'+nestedtable;
			}
			return '<row>'+cells.join('')+'</row>';
		};
		if ( config.header ) {xml += addRow( data.header );}
		for ( var i=0, ien=data.body.length ; i<ien ; i++ ) {
			xml += addRowBody( data.body[i] );
		}
...
// Extension for nested-table
var renderTimestamp = function(data) {if (data === null || data === undefined) {return '';} return new Date(data).toISOString();};
//
// PDF export - using pdfMake - http://pdfmake.org
//
DataTable.ext.buttons.pdfHtml5 = {
	className: 'buttons-pdf buttons-html5',
	available: function () {return window.FileReader !== undefined && window.pdfMake;},
	text: function ( dt ) {return dt.i18n( 'buttons.pdf', 'PDF' );},
	action: function ( e, dt, button, config ) {
		var newLine = _newLine( config );
		var data = dt.buttons.exportData( config.exportOptions );
		var rows = [];
		if ( config.header ) {
			rows.push( $.map( data.header, function ( d ) {
				return {text: typeof d === 'string' ? d : d+'', style: 'tableHeader'};
			} ) );
		}
		for ( var i=0, ien=data.body.length ; i<ien ; i++ ) {
			rows.push( $.map( data.body[i], function ( d ) {
				// Waarp nested table support
				if (Object.prototype.toString.call( d ) === '[object Array]') {
					var nestedbody = [];
					var len = d.length;
					// Put header
					var headd = d[0];
					nestedbody.push(headd);
					var fieldlen = headd.length;
					for (var k = 1; k < len ; k++) {
						var nestedrow = [];
						var rowd = d[k];
						for (var j = 0; j < fieldlen ; j++) {
							if (headd[j].search("TIME") == -1) { nestedrow.push(rowd[headd[j]]+''); }
							else { nestedrow.push(renderTimestamp(rowd[headd[j]])); }
						}
						nestedbody.push(nestedrow);
					}
					return { table: { body: nestedbody } };
				}
		...
		if ( config.footer ) {
			rows.push( $.map( data.footer, function ( d ) {
				return {text: typeof d === 'string' ? d : d+'', style: 'tableFooter'};
			} ) );
		}
		var doc = {
			...
			content: [
				{table: {headerRows: 1, body: rows}, layout: 'noBorders'}
			],
			styles: {
		...
		if ( config.message ) {
			doc.content.unshift( {text: config.message, style: 'message', margin: [ 0, 0, 0, 12 ]} );
		}
		if ( config.title ) {
			doc.content.unshift( {text: _title( config, false ), style: 'title', margin: [ 0, 0, 0, 12 ]} );
		}
		if ( config.customize ) {config.customize( doc );}
		var pdf = window.pdfMake.createPdf( doc );
		if ( config.download === 'open' && ! _isSafari() ) {pdf.open();}
		else {
