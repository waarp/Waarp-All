...
public abstract class AbstractDiskHttpData extends AbstractHttpData {
  ...
  private static final InternalLogger logger =
      InternalLoggerFactory.getInstance(AbstractDiskHttpData.class);
      ...
      try {
        ...
        if (definedSize > 0 && definedSize < size + localsize) {
          throw new IOException(
              "Out of size: " + (size + localsize) + " > " + definedSize);
        }
        int written = 0;
        if (file == null) {
          file = tempFile();
        ...
        buffer
            .getBytes(buffer.readerIndex(), fileChannel, fileChannel.position(),
                      localsize);
        size += localsize;
        buffer.readerIndex(buffer.readerIndex() + written);
      } finally {
        ...
        if (inputAccessFile != null) {
          try {
            ...
            if (exception == null) { // Choose to report the first exception
              ...
              logger.warn(
                  "Multiple exceptions detected, the following will be suppressed {}",
                  e);
            }
          }
        }
