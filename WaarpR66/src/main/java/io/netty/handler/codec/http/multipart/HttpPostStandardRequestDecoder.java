...
public class HttpPostStandardRequestDecoder
  ...
  private final List<InterfaceHttpData> bodyListHttpData =
      new ArrayList<InterfaceHttpData>();

  /**
   * HttpDatas as Map from Body
   */
  private final Map<String, List<InterfaceHttpData>> bodyMapHttpData =
      new TreeMap<String, List<InterfaceHttpData>>(
  ...
  private int discardThreshold =
      HttpPostRequestDecoder.DEFAULT_DISCARD_THRESHOLD;
   ...
   */
  public HttpPostStandardRequestDecoder(HttpDataFactory factory,
                                        HttpRequest request, Charset charset) {
  ...
  private void checkDestroyed() {
    if (destroyed) {
      throw new IllegalStateException(
          HttpPostStandardRequestDecoder.class.getSimpleName() +
  ...
  public void setDiscardThreshold(int discardThreshold) {
    this.discardThreshold =
        checkPositiveOrZero(discardThreshold, "discardThreshold");
  }
  ...
  public boolean hasNext() {
    ...
    return !bodyListHttpData.isEmpty() &&
  ...
  private void parseBodyAttributesStandard() {
    ...
    try {
        ...
        switch (currentStatus) {
          case DISPOSITION:// search '='
            if (read == '=') {
              ...
              String key = decodeAttribute(undecodedChunk.toString(firstpos,
                                                                   equalpos -
                                                                   firstpos,
                                                                   charset),
                                           charset);
          ...
          case FIELD:// search '&' or end of line
            if (read == '&') {
              currentStatus = MultiPartStatus.DISPOSITION;
              ampersandpos = currentpos - 1;
              setFinalBuffer(undecodedChunk.retainedSlice(firstpos,
                                                          ampersandpos -
                                                          firstpos));
              firstpos = currentpos;
              contRead = true;
      ...
      if (isLastChunk && currentAttribute != null) {
        ...
        firstpos = currentpos;
        currentStatus = MultiPartStatus.EPILOGUE;
      } else if (contRead && currentAttribute != null &&
                 currentStatus == MultiPartStatus.FIELD) {
  ...
  private void setFinalBuffer(ByteBuf buffer) throws IOException {
    ...
    ByteBuf decodedBuf =
        decodeAttribute(currentAttribute.getByteBuf(), charset);
    if (decodedBuf !=
        null) { // override content only when ByteBuf needed decoding
  ...
  private static ByteBuf decodeAttribute(ByteBuf b, Charset charset) {
    ...
    if (urlDecode.nextEscapedIdx != 0) { // incomplete hex byte
      ...
      buf.release();
      throw new ErrorDataDecoderException(String.format(
          "Invalid hex byte at index '%d' in string: '%s'", idx,
          b.toString(charset)));
    }
