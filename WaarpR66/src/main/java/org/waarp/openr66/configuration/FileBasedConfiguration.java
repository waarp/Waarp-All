        ...
        if (highcpuLimit > 0) {
          ...
          config.setConstraintLimitHandler(
              new R66ConstraintLimitHandler(useCpuLimit, useCpuLimitJDK,
                                            cpulimit, connlimit, lowcpuLimit,
                                            highcpuLimit, percentageDecrease,
                                            null, delay, limitLowBandwidth));
        } else {
  ...
  private static boolean loadRest(Configuration configuration) {
    ...
    try {
      ...
      if (valueRest != null && (valueRest.getList() != null)) {
        for (final XmlValue[] xml : (List<XmlValue[]>) valueRest.getList()) {
          ...
          if (config.REST_PORT > 0) {
            ...
            if (valueMethod != null && (valueMethod.getList() != null)) {
              ...
              for (final XmlValue[] xmlmethod : (List<XmlValue[]>) valueMethod
                  .getList()) {
  ...
  private static boolean loadDatabase(Configuration config, boolean initdb) {
    ...
    try {
      ...
      if (value == null || (value.isEmpty())) {
        ...
        try {
          DbConstant.admin = DbModelFactory
          ...
          if (config.getMultipleMonitors() > 1) {
            DbConstant.noCommitAdmin = DbModelFactory
                .initialize(dbdriver, dbserver, dbuser, dbpasswd, true);
          ...
          logger.info(
              ...
              " NoCommitAdmin:" + (DbConstant.noCommitAdmin != null));
