...
],atoms=wordRegexp(atomArray);CodeMirror.registerHelper("hintWords","webidl",builtinArray.concat(typeArray).concat(keywordArray).concat(atomArray));var startDefArray=["callback","dictionary","enum","interface"],startDefs=wordRegexp(startDefArray),endDefArray=["typedef"],endDefs=wordRegexp(endDefArray),singleOperators=/^[:<=>?]/,integers=/^-?([1-9][0-9]*|0[Xx][0-9A-Fa-f]+|0[0-7]*)/,floats=/^-?(([0-9]+\.[0-9]*|[0-9]*\.[0-9]+)([Ee][+-]?[0-9]+)?|[0-9]+[Ee][+-]?[0-9]+)/,identifiers=/^_?[A-Za-z][0-9A-Z_a-z-]*/,identifiersEnd=/^_?[A-Za-z][0-9A-Z_a-z-]*(?=\s*;)/,strings=/^"[^"]*"/,multilineComments=/^\/\*.*?\*\//,multilineCommentsStart=/^\/\*.*/,multilineCommentsEnd=/^.*?\*\//;function readToken(stream,state){// whitespace
if(stream.eatSpace())return null;// comment
if(state.inComment){if(stream.match(multilineCommentsEnd)){state.inComment=/* eat */ /* ignoreName */ /* ignoreName */!1/* skipSlots */ /* skipSlots */;return"comment"}stream.skipToEnd();return"comment"}if(stream.match("//")){stream.skipToEnd();return"comment"}if(stream.match(multilineComments))return"comment";if(stream.match(multilineCommentsStart)){state.inComment=!0/* skipSlots */;return"comment"}// integer and float
if(stream.match(/^-?[0-9\.]/,!1)){if(stream.match(integers)||stream.match(floats))return"number"}// string
if(stream.match(strings))return"string";// identifier
if(state.startDef&&stream.match(identifiers))return"def";if(state.endDef&&stream.match(identifiersEnd)){state.endDef=!1;return"def"}if(stream.match(keywords))return"keyword";if(stream.match(types)){var lastToken=state.lastToken,nextToken=(stream.match(/^\s*(.+?)\b/,!1)||[])[1];if(":"===lastToken||"implements"===lastToken||"implements"===nextToken||"="===nextToken){// Used as identifier
return"builtin"}else{// Used as type
return"variable-3"}}if(stream.match(builtins))return"builtin";if(stream.match(atoms))return"atom";if(stream.match(identifiers))return"variable";// other
if(stream.match(singleOperators))return"operator";// unrecognized
stream.next();return null};CodeMirror.defineMode("webidl",function(){return{startState:function startState(){return{// Is in multiline comment
...
endDef:!1}},token:function token(stream,state){var style=readToken(stream,state);if(style){var cur=stream.current();state.lastToken=cur;if("keyword"===style){state.startDef=startDefs.test(cur);state.endDef=state.endDef||endDefs.test(cur)}else{state.startDef=!1}}return style}}});CodeMirror.defineMIME("text/x-webidl","webidl")});