...
stream.next();return null};CodeMirror.defineMode("webidl",function(){return{startState:function(){return{// Is in multiline comment
...
endDef:!1}},token:function(stream,state){var style=readToken(stream,state);if(style){var cur=stream.current();state.lastToken=cur;if("keyword"===style){state.startDef=startDefs.test(cur);state.endDef=state.endDef||endDefs.test(cur)}else{state.startDef=!1}}return style}}});CodeMirror.defineMIME("text/x-webidl","webidl")});